...
TRAINING_ARGS=(
    ...
    --no-load-optim     # currently does not support load optim
    --no-load-rng       # when resumeing from a trained checkpoint, rng state should be loaded
    --seed 1234
    --lr 2.5e-8
    --min-lr 0
...
# The trainer auto-detects whether --load contains dataloader state:
#   - Found: switches to resumption mode (restores iteration, RNG, dataloader, currently not support load optim)
#   - Not found: keeps --finetune behavior (load weights only, start from iter 1)
# No manual flag changes needed.
