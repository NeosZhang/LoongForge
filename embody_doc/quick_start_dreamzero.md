6.5. quick_start_Dreamzero.md

# DreamZero 训练快速开始
本文介绍在 LoongForge 中进行 DreamZero 训练的基本流程，包括权重与数据准备、Full / LoRA 训练、续训以及可选的 Feature Cache。支持 Wan2.2-TI2V-5B 和 Wan2.1-I2V-14B 两种模型规格。

命令默认从 LoongForge 仓库根目录执行，示例使用 `/workspace/LoongForge`，请按实际环境替换。
...
完整字段以 [prepare_dataset.py](../../../tools/data_preprocess/embodied/dreamzero/prepare_dataset.py) 中的 `PRESETS` 为准。
