6.3. quick_start_xvla.md

本文档将引导您完成 LoongForge-VLA 框架下 **X-VLA** SFT（监督微调）训练的快速开始流程。

## 0. 资源准备
### 0.1 模型权重
X-VLA 主权重（Florence2 视觉-语言 backbone + SoftPromptedTransformer 动作头）由 `CHECKPOINT_PATH` 指定，并通过 `--pretrained-checkpoint` 加载，支持原始 HuggingFace safetensors 权重形态。以官方 `X-VLA-WidowX` 微调权重为例，将其下载到本地目录（请替换为实际发布的权重仓库）：

```bash
# 将官方发布的 X-VLA-WidowX checkpoint 放到该目录
hf download https://huggingface.co/2toINF/X-VLA-WidowX --local-dir /workspace/ckpt/X-VLA-WidowX
```
...
export DATA_PATH=/workspace/data/XVLA-Soft-Fold/0928_10am_new/
...
```bash
bash /workspace/AIAK-Training-Omni/examples/embodied/xvla/run_xvla_ddp_finetune.sh
```
默认已开启了优化，参数在configs/models/embodied/xvla.yaml
...
## 3. 评测
（评测流程待补充：X-VLA 推理走 LoongForge embodied eval 的共享 `predict_action` 接口，`domain_id` 由配置的 `robot_type` 经 `DOMAIN_ID_MAP` 解析。）
