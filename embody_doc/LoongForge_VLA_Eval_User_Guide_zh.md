...
|---|---|---|---|---|---|
| **pi0.5** | 任务成功（微调后） | 尚未评分 — 需要 CALVIN 领域 checkpoint | 尚未评分 — 无公开 Bridge/WidowX 微调 | 任务成功（`pi0.5_robotwin2` + `pi05_aloha_14d`） | 尚未评分 — 无匹配 checkpoint |
| **X-VLA** | 任务成功（~94% object suite） | 尚未评分 — 需要官方 ABC_D 权重 | 任务成功（X-VLA-WidowX + SimplerEnv 补丁） | 任务成功（X-VLA-RoboTwin2 + `ee6d_dual`） | 尚未评分 — 无匹配的开放权重 |

"任务成功"表示至少有一个 episode 通过了官方或本地的成功判定标准。"尚未评分"的组合仍附带可运行的 YAML（设置了 `server.random_init: true`），可用于验证 RPC 通路而无需 checkpoint。

### 1.2 可直接运行的组合

以下每个组合都有公开的 YAML，只需将其中的 `/path/to/...` 替换为实际路径即可。
...
| pi0.5 + RoboTwin | pi0.5 RoboTwin-2.0 联合微调 + 统计信息（见 *5.2 章节*） | `examples/embodied/pi05/eval/configs/robotwin/adjust_bottle_smoke.yaml` |
...
X-VLA + SimplerEnv 要获得任务成功，上游 SimplerEnv 还需要一次性补丁 — 参见 *6.3 章节* 和 `examples/embodied/xvla/eval/SIMPLERENV_PATCH_en.md`。
...
| pi0.5 + RoboTwin | `action_dim: 14`、`action_horizon: 32`；`action_bridge: pi05_aloha_14d`；`dataset_statistics_path`（见 *5.2 章节*） |
...
GPU 要求：模型推理需要至少 1 张 NVIDIA GPU（显存 ≥16 GB，推荐 A100/A800）。基准测试侧渲染（SimplerEnv、RoboTwin、ManiSkill）同样依赖 GPU 的 Vulkan 驱动。
...
model:
  backend: loongforge       # 或 `mock` 用于仅 RPC 冒烟测试
...
| `model.backend` | `loongforge` 启动真实模型服务器。`mock` 启动仅协议服务器用于 RPC 冒烟测试。 |
...
| `server.random_init: true` | 跳过 checkpoint 加载；在没有领域权重时有用。 |
...
examples/embodied/<model>/eval/
  configs/
    <benchmark>/*.yaml     # 每个基准测试一个公开 YAML
  run_<benchmark>_eval.sh  # 启动脚本
```

所有发布的 YAML 使用 `/path/to/...` 占位符。请勿提交机器特定的绝对路径。
...
RoboTwin 通过官方 `script/eval_policy.py` 启动；桥接代码位于 `loongforge/embodied/eval/bridges/robotwin_policy.py`。使用的协议由 `benchmark.action_bridge`（或对于自行选择的模型，`model.robotwin_action_bridge`）决定 — 仅在 YAML 中设置。
...
生成脚本（openpi 格式 checkpoint → LoongForge 统计信息）：
...
公开 YAML 也支持连通性检查。编辑同一 YAML：

- `server.random_init: true`、`max_steps: 5` — 随机 14D 输出，验证官方 runner 可达。
- `action_bridge: duplicate_7d`（配合 `model.action_dim: 7`） — 仅接口检查。非真实分数。
- `action_bridge: strict_14d` — 原始 14D 关节，无 `adapt_to_pi`。非 openpi 正式协议。

### 4.3 SimplerEnv

X-VLA + SimplerEnv 是任务成功组合。pi0.5 尚无公开 Bridge 或 WidowX 微调，因此其 YAML 以 `random_init: true` 运行仅用于连通性验证。

发布的 YAML：

- pi0.5（随机初始化）：`examples/embodied/pi05/eval/configs/simplerenv/widowx_stack_cube_smoke.yaml`
- X-VLA（任务成功）：`examples/embodied/xvla/eval/configs/simplerenv/widowx_stack_cube_smoke.yaml`

通过同一 YAML 中的 `task_name`、`robot_setup` 和 `scene_name` 切换 Bridge 任务（eggplant、carrot、spoon 等）。运行器在导入 SAPIEN 之前会重新执行当前进程（re-exec），以确保 `LD_LIBRARY_PATH` 和 `VK_ICD_FILENAMES` 生效。

X-VLA 要获得任务成功，上游 SimplerEnv 还需要一次性补丁以启用绝对 EE 控制 — 见 *6.3 章节*。
...
pi0.5 和 X-VLA 目前均无 CALVIN 领域 checkpoint，因此发布的 YAML 使用 `server.random_init: true` 仅用于连通性验证：

- `examples/embodied/pi05/eval/configs/calvin/smoke.yaml`
- `examples/embodied/xvla/eval/configs/calvin/smoke.yaml`

有 CALVIN 领域 checkpoint 时，设置 `random_init: false` 并填写 `ckpt_path` / `dataset_statistics_path`。对于 X-VLA，正式协议为 `domain_id: 2`、`action_postprocess: ee6d_to_calvin_abs`。

### 4.5 ManiSkill

ManiSkill 是基于 SAPIEN 的 GPU 友好操作套件。目前无公开开放权重组合有分数，因此发布的 YAML 以 `random_init: true` 运行 PickCube：
...
默认值：`PickCube-v1`、`pd_ee_delta_pose`、7D 动作。在 YAML 中修改 `task` 和 `obs_mode`（rgbd 或 state）。
...
> **注意：这不同于 RLinf PutOnPlate。** RLinf 的 pi0.5 ManiSkill SFT 训练在 `PutOnPlateInScene25Main-v3`（WidowX Bridge real2sim）上，而非标准 `PickCube-v1` + Panda。LoongForge 不提供该环境；PickCube YAML 仅用于验证 ManiSkill runner 和 RPC 通路。
...
磁盘紧张或仅做接口检查时，设置 `run.save_replay: false` 和 `run.save_trace: false`，仅保留结果文件和策略日志。
...
`run.output_dir` 应指向一个稳定的运行标签目录，如 `reports/pi05/robotwin/adjust_bottle_smoke`。统一入口默认 `run.timestamped_output: true`，在父目录下创建 `<yyyymmdd_hhmmss>_<run_tag>`。`reports/` 仅本地使用，不提交。调试时若要重用固定目录，设置 `run.timestamped_output: false`。
...
本节面向需要集成新模型的开发者,如需要集成 pi0.5 / X-VLA 之外的模型，核心思路是：复用共享的 `predict_action` 接口和 `GenericPredictActionPolicy`，保持基准测试协议和适配器不变，将模型差异放在一个轻量工厂中。不要 fork 基准测试运行器或修改训练树中的 LoongForge 源码。详细的集成步骤、模型语义对比（动作空间、归一化归属、chunk 长度等）和最小检查清单，请参见：
