...
本文档面向使用 LoongForge-VLA 进行具身模型训练、微调的用户，聚焦框架级通用能力，说明各模块支持的功能、配置入口、默认值、可选项与使用方式。具体模型的专用训练方案、数据预处理脚本、模型私有性能开关在对应的** Quick Start **文档中维护。
...
```
### 1.3 配置分层
LoongForge-VLA 将配置分为三类对象：

|配置对象|配置入口|作用域|示例|
|-|-|-|-|
|TrainingArgs|shell flag|模型无关的训练运行参数|--train-iters<br/>--lr-base<br/>--distributed-strategy|
|ModelConfig|YAML 或 model.xxx=...|模型结构与模型侧训练开关|model.action_horizon=64|
|DataConfig|YAML 或 data.xxx=...|数据处理参数|data.image_size=256|
...
```
## 2. 数据处理
数据处理模块负责：将数据集样本转换为模型可直接消费的 PreparedBatch，用户通过 TrainingArgs 选择数据后端与 DataLoader 行为，通过 DataConfig 配置模型相关的数据处理逻辑。
...
|功能|配置项|默认值|取值范围 / 形式|说明|
|-|-|-|-|-|
|数据格式|--dataset-format|lerobot_datasets|lerobot_datasets<br/>hdf5_datasets<br/>dummy_datasets|选择读取的数据格式|
|数据路径|--dataset-path|None|本地路径或数据集 id|训练数据位置|
|数据 split|--split|train|字符串|数据切分|
|dummy 样本数|--num-samples|100|正整数|dummy_datasets 下生成的样本数|

### 2.2 LeRobot 数据策略
使用 LeRobot 格式时，可通过以下参数进一步控制数据加载行为：指定数据集格式版本（v2.0 / v2.1 / v3.0）、选择针对不同机器人或任务的构建策略、配置视频解码后端，以及指定 robot 类型以匹配 embodiment 的 action-state 布局。

|功能|配置项|默认值|取值范围 / 形式|说明|
|-|-|-|-|-|
|LeRobot 版本|--lerobotdataset-version|v3.0|v2.0、v2.1、v3.0|解析不同 LeRobot 磁盘格式|
...
### 2.3 DataLoader 行为
Worker 数影响数据预取吞吐；多进程上下文（fork / spawn / forkserver）可解决特定库的进程兼容问题；分布式采样支持 cyclic（交错）和 block（连续块）两种 index 切分方式；流式模式适用于超大规模数据集（--lerobotdataset-version v3.0格式）
...
* 通过 `--config-file` 覆盖默认 YAML，使用自定义配置文件
...
### 3.2 训练基础参数
支持通过 max_steps 与 per_device_batch_size 声明式控制训练规模；基于梯度累积（Gradient Accumulation）实现全局 Batch 的弹性扩缩；通过固定随机种子保障初始化与数据采样的确定性复现；并以输出目录为唯一根路径，统一归管运行态产物（日志、Checkpoint 及中间文件）。

|功能|配置项|默认值|取值范围 / 形式|说明|
|-|-|-|-|-|
|训练总步数|--train-iters|150000|正整数|optimizer update 步数|
|每设备 batch|--per-device-batch-size|4|正整数|单个 rank 每次 forward 的 micro-batch 大小|
...
|优化器|--optimizer|AdamW|见下表|optimizer 实现|
...
* --lr-decay-style 支持 10 种调度策略，涵盖线性、cosine、polynomial、恒定及带 warmup / min_lr 变体：
...
* --optimizer 支持标准 AdamW、多种融合加速实现（PyTorch / TransformerEngine / Apex）及 Adam、SGD：
...
|功能|配置项|默认值|取值范围 / 形式|说明|
|-|-|-|-|-|
|模块级学习率|--lr-group|None|module.path=lr,module.path=lr|为指定模块设置独立学习率|
...
* 支持 safetensors、pt、dcp 三种格式权重保存；支持 DCP 格式异步保存；
* 支持完整训练状态保存（optimizer / scheduler / RNG / DataLoader），支持断点续训；
...
|W&B 项目|--wandb-project|loongforge-vla|字符串|W&B project 名称|
...
|功能|配置项|默认值|取值范围 / 形式|说明|
|-|-|-|-|-|
|冻结模块|--freeze-modules|""|逗号分隔模块路径|将匹配模块参数设置为 requires_grad=False|

## 4. 分布式 Trainer
Trainer 模块负责训练生命周期编排，包括分布式上下文初始化、模型构建、权重加载、模型包装、优化器与 scheduler 构建、DataLoader 构建、训练循环、日志、checkpoint 与资源清理。

### 4.1 Trainer 选择
通过 --trainer-type 选择训练器，默认使用 FinetuneTrainer；可选值为 trainer_builder.py 中注册的 Trainer 类名，用于决定训练范式。使用建议：

|Trainer|适用场景|
|-|-|
|FinetuneTrainer|标准单数据流监督微调|
|自定义 Trainer|多数据流、特殊 loss 组合或特殊 step 调度|
