...
(EMBODIED_BASELINE_ROOT is centrally configured by config/env.sh, defaulting outside the repo at
${EMBODIED_CI_ROOT}/metrics_baseline, to avoid multiple checkouts overwriting each other.)
...
def _baseline_root():
    root = os.environ.get("EMBODIED_BASELINE_ROOT")
    if not root:
        raise RuntimeError(
            "EMBODIED_BASELINE_ROOT is not set; please source config/env.sh first"
        )
    return root
