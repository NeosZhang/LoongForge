...
Usage examples:
    python3 cli/manual.py --models pi05 groot_n1_6
    python3 cli/manual.py --auto_collect_baseline          # collect baseline on first run
    python3 cli/manual.py --dry_run                        # only print commands, do not train
"""
...
# This file lives under tests/cli/; TESTS_ROOT points to tests/ (the package root for execution / reporting).
...
def _ensure_env_loaded():
    ...
    cli/manual.sh sources env.sh before exec-ing this file; when this file is
    called directly (python3 cli/manual.py ...), env.sh has not been sourced
    ...
    if os.environ.get("EMBODIED_CI_ROOT"):
        return
    env_sh = os.path.join(TESTS_ROOT, "config", "env.sh")
...
# defaults below reflect the same paths that cli/manual.sh would have sourced.
_ensure_env_loaded()
...
def parse_args(available_models):
    ...
    # --- Env-var defaults (mirror the old cli/manual.sh conventions) ---
...
def main():
    ...
    if args.prepare:
        prepare_sh = os.path.join(TESTS_ROOT, "config", "prepare.sh")
    ...
    scripts_root = examples_dir(TESTS_ROOT)
    ...
    for index, model_name in enumerate(models, 1):
        ...
        result = run_script(model_name, script_path, args, TESTS_ROOT,
                            args.log_dir, logger)
