...
def run_script(model_name, script_path, args, tests_root, log_dir, logger):
    """Execute one regression script and return a result dict.

    Note: the tests_root parameter is kept for compatibility with existing callers, but is
    currently not actually used (the baseline path is determined by the EMBODIED_BASELINE_ROOT environment variable).
    """
    del tests_root  # unused
