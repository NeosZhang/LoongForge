...
- **Trigger `cli/manual.sh`** (inside the container): a thin wrapper that only sources `config/env.sh`
  (so path variables propagate to training subprocesses), then delegates to `cli/manual.py`.
- **Executor `cli/manual.py`**: owns everything else — parameter parsing / defaults / validation,
...
Calling `cli/manual.py` directly is equivalent — it auto-sources `config/env.sh` on first use.

## Workflow

```
cli/manual.sh  (thin wrapper: source config/env.sh, then exec python3 cli/manual.py "$@")
  └─ cli/manual.py (execution layer)
       ...
       5. metric parsing (metrics.jsonl preferred, fall back to stdout)
            → auto_collect_baseline=true: only collect and write to metrics_baseline, no comparison
...
you invoke via `cli/manual.sh` or `cli/manual.py`). To switch machines, you only need to change
this one file, or temporarily override with environment variables before the command:

- `EMBODIED_CI_ROOT` — default `/ssd2/loongforge_embodied_ci`; unified root directory, the paths below are all derived from it by default
...
- `EMBODIED_BASELINE_ROOT` — default `${EMBODIED_CI_ROOT}/metrics_baseline`; baseline root directory (outside the repo)
...
- **Data and weights** `vla_artifacts/`: organized as `<family>/{models,datasets,tokenizers}`; the training scripts' default paths are derived from
  `LOCAL_VLA_ARTIFACTS_ROOT`. When files are missing or switching machines, use `bash config/prepare.sh` to sync from BOS.
...
- **baseline** `metrics_baseline/<chip>/<name>.json` (**critical**): missing baseline means a direct FAIL. Whichever chip you run on,
  first collect all models in the manifest on that chip:
  ```bash
  chip=A800 auto_collect_baseline=true bash loongforge/embodied/tests/cli/manual.sh
  ```
...
```
loongforge/embodied/tests/
├── cli/                        # entry scripts
│   ├── manual.sh               #   thin wrapper: source config/env.sh, then exec manual.py
│   └── manual.py               #   regression executor: parameter parsing + serial execution per manifest
...
└── reporting/                  # result layer: baseline comparison, logging
    ├── baseline.py             #   baseline read/write, tolerance comparison, write back baseline on performance improvement
    └── log.py                  #   colored console logging

# The following are mutable state/artifacts, placed by default outside the repo under ${EMBODIED_CI_ROOT} (see config/env.sh):
${EMBODIED_CI_ROOT}/
├── metrics_baseline/<chip>/  # per-model baseline JSON (EMBODIED_BASELINE_ROOT)
└── logs/run_<timestamp>/     # per-run logs + results.json (EMBODIED_LOG_ROOT)
```

## Usage

Two entry points divide the work: `config/prepare.sh` prepares the environment (one-off/idempotent), and `cli/manual.sh`
runs the regression inside the container.

```bash
# Full regression (inside the container)
bash loongforge/embodied/tests/cli/manual.sh --chip A800

# Run only one script (name from config/scripts.yaml) — choose either environment variable or command-line flag
chip=A800 model_names="pi05" bash loongforge/embodied/tests/cli/manual.sh
bash loongforge/embodied/tests/cli/manual.sh --chip A800 --models pi05

# Run multiple scripts
chip=A800 model_names="pi05 groot_n1_6" bash loongforge/embodied/tests/cli/manual.sh
bash loongforge/embodied/tests/cli/manual.sh --chip A800 --models "pi05 groot_n1_6"

# First-time baseline collection (no comparison, write the current result into metrics_baseline/<chip>/)
chip=A800 auto_collect_baseline=true bash loongforge/embodied/tests/cli/manual.sh
bash loongforge/embodied/tests/cli/manual.sh --chip A800 --auto_collect_baseline

# Prepare the environment on-site (bos sync data), skipped by default, needs explicit enabling
bash loongforge/embodied/tests/cli/manual.sh --chip A800 --prepare

# Skip the grad_norm hard check and only enforce loss (both are checked by default)
bash loongforge/embodied/tests/cli/manual.sh --chip A800 --check_loss_only

# Only print commands without training, to verify the pipeline
chip=A800 dry_run=true bash loongforge/embodied/tests/cli/manual.sh
bash loongforge/embodied/tests/cli/manual.sh --chip A800 --dry_run

# Stop at the first FAIL (skip the remaining models, useful for debugging)
bash loongforge/embodied/tests/cli/manual.sh --chip A800 --fail_fast

# List available names
bash loongforge/embodied/tests/cli/manual.sh --chip A800 --list_models
```

## Parameter Interface

`cli/manual.sh` is a thin wrapper that only sources `config/env.sh` (to propagate path variables
to the training subprocesses) and delegates all parameter parsing, defaults, validation, and
optional `--prepare` execution to `cli/manual.py`. Calling `cli/manual.py` directly is equivalent —
...
|---|---|---|---|
| `model_names` | `--models MODEL ...` | empty (=all) | Names to regress (must be in `config/scripts.yaml`); env-var form is a space-separated string, e.g. `model_names="pi05 groot_n1_6"` |
| `chip` | `--chip` | **required** | Chip model, `A800` / `P6K`; determines the baseline subdirectory `metrics_baseline/<chip>/` |
...
- Regression passes and iteration time/throughput **improves** beyond that tolerance (with no performance metric degradation) -> automatically write back
  the performance metrics in `metrics_baseline` to this run's better values (accuracy metrics are never auto-updated)
- Missing `metrics_baseline/<chip>/<name>.json` -> **preflight refusal**: `cli/manual.py` checks all selected
