...
def build_param_groups(model: nn.Module, args) -> List[Dict]:
    ...
    LR assignment priority (highest to lowest):
      ...
      2. ``--lr-backbone`` / ``--lr-action-model``  — legacy named overrides
         with hardcoded candidate paths. Used only when ``--lr-group`` is
         not set.
      3. ``--lr-base``  — fallback for all remaining trainable parameters.
    ...
    # Build lr_mappings: --lr-group takes priority over legacy args
    ...
    if lr_group_str:
        ...
        # Process action model before backbone: gemma_expert is a submodule of
        # paligemma_with_expert, so action model params must be consumed first.
        # If backbone ran first, paligemma_with_expert would absorb all its
        # descendant params (including gemma_expert), leaving nothing for the
        # action model group.
