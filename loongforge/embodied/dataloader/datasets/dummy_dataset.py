...
Usage:
    datasets:
      vla_data:
        dataloader_module: dummy_datasets
        num_samples: 100
        action_dim: 7
        action_horizon: 7
        image_size: 224
...
def build_dummy_dataloader(cfg, vla_cfg, batch_size: int, num_workers: int) -> DataLoader:
    """Build Dummy dataloader."""
    num_samples = vla_cfg.get("num_samples", 100)
    action_dim = vla_cfg.get("action_dim", 7)
    state_dim = vla_cfg.get("state_dim", 7)
