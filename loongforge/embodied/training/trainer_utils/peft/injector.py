...
# (attr_name on model or model.architecture)
...
def apply_lora(
    ...
    *,
    ...
    if spec.vlm_module:
        # Try on model directly, then on model.architecture
    ...
    # The VLM interface may wrap the actual model as .model
    if hasattr(vlm_interface, "model") and isinstance(vlm_interface.model, nn.Module):
        ...
        # Apply directly if it's a bare nn.Module
