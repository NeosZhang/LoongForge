...
Florence-2 implementation with DAViT vision encoder and BART language encoder.
Provides image encoding, language encoding, and multimodal fusion capabilities.
"""
...
from loongforge.embodied.model.xvla.configuration_florence2 import Florence2Config, Florence2LanguageConfig
from loongforge.embodied.model.xvla.utils import drop_path
...
class MySequential(nn.Sequential):
    """Sequential module that supports multi-argument forward pass."""
...
class PreNorm(nn.Module):
    """Pre-normalization wrapper that applies normalization before a given function."""

    def __init__(self, norm, fn, drop_path=None):
        """Initialize PreNorm with norm layer, function, and optional drop path."""
        super().__init__()
        self.norm = norm
        self.fn = fn
    ...
    def forward(self, x, *args, **kwargs):
        """Apply pre-normalization and function, with optional drop path."""
        shortcut = x
...
class Mlp(nn.Module):
    """Multi-layer perceptron with configurable hidden size and activation."""
    ...
    def forward(self, x, size):
        """Apply the network and return output with size."""
...
class DepthWiseConv2d(nn.Module):
    ...
    def forward(self, x, size):
        """Apply depthwise 2D convolution to the input feature map."""
...
class ConvEmbed(nn.Module):
    ...
    def forward(self, x, size):
        """Apply convolutional patch embedding to input features."""
        height, width = size
        if len(x.size()) == 3:
...
class ChannelAttention(nn.Module):
    """Channel-wise multi-head attention module."""
...
class ChannelBlock(nn.Module):
    """Channel-wise attention block with optional depthwise convolutions."""
    ...
    def __init__(
        ...
        """Initialize ChannelBlock with channel attention, feed-forward, and optional convolutions."""
        super().__init__()
    ...
    def forward(self, x, size):
        """Forward pass through channel block with optional convolutions."""
        if self.conv1:
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size, qkv_bias=True):
        """Initialize WindowAttention with projection layers and softmax."""
        super().__init__()
        self.dim = dim
...
class SpatialBlock(nn.Module):
    """Spatial attention block combining window attention and channel feed-forward."""
    ...
    def __init__(
        ...
        """Initialize SpatialBlock with window attention, feed-forward, and optional convolutions."""
        super().__init__()
    ...
    def forward(self, x, size):
        """Forward pass through spatial block with optional convolutions."""
        if self.conv1:
            x, size = self.conv1(x, size)
...
class DaViT(nn.Module):
    ...
    @property
    def dim_out(self):
        """Return output dimension (last embed dim)."""
    ...
    def forward_features(self, x):
        """Extract features with average pooling."""
    ...
    def forward(self, x):
        """Forward pass through DaViT model."""
    ...
    def from_config(cls, config):
        """Create DaViT instance from configuration."""
        return cls(
...
class Florence2ScaledWordEmbedding(nn.Embedding):
    ...
    def forward(self, input_ids: torch.Tensor):
        """Forward pass scaling embeddings by embed_scale."""
...
class Florence2FlashAttention2(Florence2Attention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # bottom-right alignment, that was made default for flash_attn>=2.1.
...
class Florence2SdpaAttention(Florence2Attention):
    """Florence2 attention using scaled dot product attention (SDPA)."""
    ...
    def forward(
        ...
        # non-contiguous inputs and a custom attn_mask, but we are fine here as `_shape` do call
        # `.contiguous()`. Reference: https://github.com/pytorch/pytorch/issues/112577
...
class Florence2EncoderLayer(nn.Module):
    """Single encoder layer for Florence2 with self-attention and feed-forward."""

    def __init__(self, config: Florence2LanguageConfig):
        """Initialize Florence2EncoderLayer with attention and feed-forward layers."""
        super().__init__()
...
class Florence2DecoderLayer(nn.Module):
    """Single decoder layer for Florence2 with self-attention, cross-attention, and feed-forward."""

    def __init__(self, config: Florence2LanguageConfig):
        """Initialize Florence2DecoderLayer with self-attention, cross-attention, and feed-forward layers."""
        super().__init__()
        self.embed_dim = config.d_model
...
class Florence2LanguagePreTrainedModel(PreTrainedModel):
    """Base class for Florence2 language models with pre-trained weight handling."""
    ...
    def dummy_inputs(self):
        """Return dummy inputs for model initialization."""
    ...
    def forward(
        ...
        if encoder_hidden_states is not None and encoder_attention_mask is not None:
            if self._use_flash_attention_2:
                ...
                # output_attentions=True & cross_attn_head_mask can not be supported when using SDPA,
                # and we fall back on the manual implementation that requires a 4D causal mask in all cases.
                # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
...
class Florence2LanguageModel(Florence2LanguagePreTrainedModel):
    """Florence2 language model with encoder-decoder architecture."""
    ...
    def __init__(self, config: Florence2LanguageConfig):
        """Initialize Florence2LanguageModel with shared embeddings, encoder, and decoder."""
        super().__init__(config)
    ...
    def _tie_weights(self):
        """Tie encoder embedding weights if configured."""
        if self.config.tie_word_embeddings:
            self._tie_or_clone_weights(self.encoder.embed_tokens, self.shared)
            # self._tie_or_clone_weights(self.decoder.embed_tokens, self.shared)

    def get_input_embeddings(self):
        """Return shared input embedding layer."""
        return self.shared

    def set_input_embeddings(self, value):
        """Set shared input embedding layer and update encoder/decoder."""
        self.shared = value
    ...
    def forward(
        ...
        """Forward pass through encoder-decoder language model."""
...
class Florence2LanguageForConditionalGeneration(Florence2LanguagePreTrainedModel, GenerationMixin):
    """Florence2 language model with conditional generation head."""
    ...
    def _tie_weights(self):
        """Tie encoder and decoder embedding weights if configured."""
        if self.config.tie_word_embeddings:
    ...
    def resize_token_embeddings(
        ...
        """Resize token embeddings and update final logits bias accordingly."""
        new_embeddings = super().resize_token_embeddings(new_num_tokens, pad_to_multiple_of, **kwargs)
        self._resize_final_logits_bias(new_embeddings.weight.shape[0])
        return new_embeddings

    def _resize_final_logits_bias(self, new_num_tokens: int) -> None:
        """Resize the final logits bias buffer to match new number of tokens."""
    ...
    def get_output_embeddings(self):
        """Return the output embedding (language model head) layer."""
        return self.lm_head

    def set_output_embeddings(self, new_embeddings):
        """Set the output embedding (language model head) layer."""
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        """Prepare inputs for autoregressive generation (language model only)."""
...
class Florence2Seq2SeqLMOutput(ModelOutput):
    """
    Base class for Florence-2 model's outputs that also contains : pre-computed hidden states that
    can speed up sequential decoding.
...
class Florence2PreTrainedModel(PreTrainedModel):
    """Base class for Florence2 multimodal models."""
...
FLORENCE2_INPUTS_DOCSTRING = r"""
    Args:
        ...
        past_key_values (`tuple(tuple(torch.FloatTensor))`, *optional*, returned when
            ...
            tensors of shape `(batch_size, num_heads, sequence_length, embed_size_per_head)`) and 2
            additional tensors of shape
            `(batch_size, num_heads, encoder_sequence_length, embed_size_per_head)`.
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    """Florence2 model for conditional text generation with visual inputs."""
    ...
    def __init__(self, config: Florence2Config):
        """Initialize Florence2ForConditionalGeneration with vision tower and language model."""
        super().__init__(config)
    ...
    def _build_image_projection_layers(self, config):
        """Build image projection layers including position embeddings and temporal embeddings."""
    ...
    def get_encoder(self):
        """Return the language model encoder."""
    ...
    def get_input_embeddings(self):
        """Return input embeddings from the language model."""
        return self.language_model.get_input_embeddings()

    def resize_token_embeddings(
        self, new_num_tokens: int | None = None, pad_to_multiple_of=None, **kwargs
    ) -> nn.Embedding:
        """Resize token embeddings and update vocabulary size in config."""
    ...
    def generate(self, input_ids, inputs_embeds=None, pixel_values=None, **kwargs):
        """Generate text conditioned on input ids and optional image pixel values."""
        if inputs_embeds is None:
            # 1. Extra the input embeddings
            if input_ids is not None:
                inputs_embeds = self.get_input_embeddings()(input_ids)
    ...
    def prepare_decoder_input_ids_from_labels(self, labels: torch.Tensor):
        """Shift labels right to create decoder input IDs using language model."""
