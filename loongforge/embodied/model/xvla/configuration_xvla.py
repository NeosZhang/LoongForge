...
Defines model architecture, training parameters, and input/output feature contracts.
"""
#!/usr/bin/env python
...
from loongforge.embodied.model.xvla.util.types import FeatureType, NormalizationMode, PolicyFeature
from loongforge.embodied.model.xvla.util.policies import PreTrainedConfig
from loongforge.embodied.model.xvla.util.optimizers import XVLAAdamWConfig
from loongforge.embodied.model.xvla.util.schedulers import CosineDecayWithWarmupSchedulerConfig
from loongforge.embodied.model.xvla.util.constants import OBS_IMAGES
from loongforge.embodied.model.xvla.util.import_utils import _transformers_available

if TYPE_CHECKING or _transformers_available:
    from loongforge.embodied.model.xvla.configuration_florence2 import Florence2Config
else:
...
class XVLAConfig(PreTrainedConfig):
    ...
    def validate_features(self) -> None:
        """Validate and normalize feature config; fill in derived fields like num_image_views."""
        ...
        if self.empty_cameras > 0:
            ...
            if self.resize_imgs_with_padding is not None:
                height, width = tuple[int, int](self.resize_imgs_with_padding)
    ...
    def get_scheduler_preset(self) -> CosineDecayWithWarmupSchedulerConfig:
        """Return the cosine decay with warmup scheduler config."""
    ...
    def observation_delta_indices(self) -> list[int] | None:
        """Return observation delta indices (None for XVLA)."""
        return None

    @property
    def action_delta_indices(self) -> list[int]:
        """Return action delta indices as range over chunk size."""
        return list(range(self.chunk_size))

    @property
    def reward_delta_indices(self) -> list[int] | None:
        """Return reward delta indices (None for XVLA)."""
        return None
