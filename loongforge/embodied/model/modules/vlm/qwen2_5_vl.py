# Adapted from 2025 starVLA community
# Adapted for LoongForge framework.

"""Qwen2.5-VL interface module for LoongForge.

Provides a unified wrapper (:class:`Qwen25VLInterface`) around
``Qwen2_5_VLForConditionalGeneration`` so that downstream training and inference
code can interact with the vision-language model through a consistent
``forward`` / ``generate`` / ``build_qwenvl_inputs`` API.

Typical usage:
    from loongforge.embodied.model.modules.vlm import get_vlm_model
    vlm = get_vlm_model(config)
    batch = vlm.build_qwenvl_inputs(images=images, instructions=instructions)
"""
...
# Action token range (canonical source: loongforge.embodied.model.compose.action.fast_action)
from loongforge.embodied.model.compose.action.fast_action import (  # noqa: E402
    _ACTION_TOKEN_MIN,
    _ACTION_TOKEN_MAX,
)


class Qwen25VLInterface(nn.Module):
    """
    LoongForge wrapper for Qwen2.5-VL (Qwen2_5_VLForConditionalGeneration).
    ...
    Notes:
        ...
        - Adapted for LoongForge configuration structure.
    """
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        Parameters:
            config (dict | Any | None):
                Expected to have backbone configuration with:
                    backbone.base_vlm (str): HuggingFace model id or local path.
                    backbone.attn_implementation (str): Attention implementation.
            **kwargs:
                Ignored currently; placeholder for future extension.

        Side Effects:
            - Downloads / loads pretrained Qwen2.5-VL weights (unless cached).
            - Instantiates AutoProcessor and enforces left padding.
        """
        super().__init__()

        # Read base_vlm from YAML config
        backbone_cfg = config.get("backbone", {})
        model_id = backbone_cfg.get("base_vlm", "Qwen/Qwen2.5-VL-3B-Instruct")
        ...
        # Resize embeddings for action tokens (151665-153712)
        # Qwen2.5-VL-3B-Instruct-Action extends vocab with action tokens
        new_vocab_size = _ACTION_TOKEN_MAX + 1  # 153713
    ...
    def forward(
        ...
        **kwargs,
        ...
        Args:
            ...
            image_grid_thw (FloatTensor | None): Optional tiling metadata
                (e.g., [B, 3] for temporal/height/width splits).
    ...
    def build_qwenvl_inputs(self, images, instructions, solutions=None, **kwargs):
        ...
        Shapes / Notes:
            ...
            - pixel_values may have internal batching distinct from B if images are flattened;
                underlying model maps them.
        ...
        for imgs, instruction in zip(images, instructions):
            ...
            # CoT_prompt support: read from backbone config or use instruction directly
        ...
        if solutions is not None:
            ...
            for i in range(labels.size(0)):
                ...
                if nonzero_indices.numel() > 0:
                    ...
                    seq[:] = IGNORE_INDEX
                    RuntimeWarning(
                        "action token not found in your tokenizer"
                        "please download QwenVL model with Action token from"
                        "https://huggingface.co/StarVLA/Qwen2.5-VL-3B-Instruct-Action"
                    )
