...
def flash_mla_sparse_bwd(
    ...
    Args:
        ...
        lse: [s_q, h_q], float32 - Log-Sum-Exp (from forward, without sink contribution)
        ...
        attn_sink: optional, [h_q], float32 - Per-head attention sink logit (same as used in forward).
            If provided, backward accounts for the scale = exp(lse) / (exp(lse) + exp(attn_sink))
